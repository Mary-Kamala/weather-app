...
export default function HomePage() {
  ...
  //  Load default location from db.json when app starts
  useEffect(() => {
    const fetchDefault = async () => {
      try {
        const res = await fetch("http://localhost:4000/defaultLocation");
        const data = await res.json();
        if (data.length > 0) {
          dispatch(getWeather(data[0].city));
        }
      } catch (error) {
  ...
  return (
    <div>
      ...
      {/* 🏙️ Show previously searched cities */}
      <Tabs />
    </div>
  );
}
